...
<div id="" style="border:0px solid; width:570px; height:350px; margin-left:5px;">
    <div style="border-bottom:1px solid #EBEBEB; padding:20px 20px;
    background-color:#f7f7f7;
    ">
       编辑菜单 (<%=request.getParameter("name") %>)
    </div>
    <div style="margin-top:20px;">
        <table border='0' style="margin-left:20px">
            ...
            <tr id="isHaveSub" style="display: none;">
                        <td style="border: 0px solid red; width: 130px; height: 50px;">
                            	点击菜单触发:
                        </td>
                        <td style="border: 0px solid red; width: 200px; height: 50px;">
        ...
        margin-top:20px; border-top:1px solid #EBEBEB;">
            ...
            background-color:#0076CC;" value="确认" onclick="updMenu()">
        </div>
        <div id="add" style=" width:580px; height:50px; display:none;
        margin-top:20px; border-top:1px solid #EBEBEB;">
            <input type="button" style="height:40px; width:80px; 
            margin:10px 0 0 240px;
            background-color:#0076CC;" value="确认" onclick="addMenu('')">
        </div>
    </div>
</div>

<input type="hidden" id="menuid" value="<%=request.getParameter("id") %>">
<input type="hidden" id="menuState" value="<%=request.getParameter("state") %>">
...
$(function() {
    ...
    if($('#menuState').val() == 'closed'){
    	document.getElementById('isHaveSub').style.display = 'block';
    	if($('#menuType').val() == '0'){
